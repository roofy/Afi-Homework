...
\begin{frame}[t]
    ...
    Richtung ''$\sum_{k=1}^{q} \frac{1}{r^{\ell_k}} \leq 1 \,\Longrightarrow\,$ $\mathcal{C}$ existiert''.\\[10pt]
    \pause
...
\begin{frame}[t]
    ...
    \begin{column}{0.5\textwidth}
        \visible<1->{
            Sei also $i = 1$. Wähle Knoten $v_w$ der Höhe $\ell_1 > 0$ beliebig und setze $w_i = w_1 := w$.\\[10pt]\pause
            Setze $h := \ell_q$ (max. Wortlänge) und
            $\mathcal{T}_0 := \mathcal{T}_r^h,\ \mathcal{T}_1 := \mathcal{T}_r^h \setminus v_{w_1}$.\\[10pt]
        }\pause
        $\mathcal{T}_1$ noch
        $r^h - r^{h - \ell_1}$ Blätter.\\
        \visible<4->{
            Weiter gilt:
            $$
                r^h - r^{h - \ell_1} = r^h\left(1 - \sum_{k=1}^{1} \frac{1}{r^{\ell_k}}\right)
            $$
            \visible<5->{
                $$
                    > r^h\left(1 - \sum_{k=1}^{q} \frac{1}{r^{\ell_k}}\right) \visible<6>{\geq 0}
                $$
            }
        }
    \end{column}
...
\begin{frame}[t]
    ...
    \only<2->{
        ...
        durch Wahl von $w_{i+1}$ ein Präfix-Code ($\ell_j \leq \ell_{i+1}$, also $w_{i+1} \not\sqsubseteq w_j$).
    }
    \pause
    \only<3->{
        \\[10pt]
    ...
    \frametitle{Ungleichung von Kraft: ''$\,\Longleftarrow\,$''}
        \begin{center}\onslide
            \begin{tikzpicture}
                ...
                \only<3->{
                    \draw[|-|, line width = .8pt] (-1,-4.6) -- node[below] {$r^{h-\ell_i}$} (2,-4.6);
                }
            \end{tikzpicture}
        \end{center}
        \visible<4->{
