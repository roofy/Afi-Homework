...
\begin{itemize}
    ...
    \item Non-deterministische Turingmaschinene (NTM's)
...
Die Wort Case Laufzeit eines Algorithmus $A$ mit die maximalen Laufzeitkosten
...
Die mögl. Rechenwege einer NTM können in einem Berechnungsbaum zsmgefasst werden. Dabei entsprechen
Knoten Konfigurationen, die Wurzel der Startkonfig., die Kinder einer Konfig alle mögl. Nachfolgekonfigurationen.
...
Eine NTM $M$ akzeptiert eine Eingabe $x \in \Sigma^*$, falls es \textbf{mindestens} einen Rechenweg
von $M$, indem $M$ die Eingabe $x$ akzeptiert (im Sinne einer normalen TM). Dann ist in diesem Sinne:
$$
...
\begin{itemize}
    ...
    \item Es gibt ca. $2^{p(|x|)}$ von diesen, und $V$ ist polynomiell in $|x| + |y|$
